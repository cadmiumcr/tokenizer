  ...
  context "Language: English (en)" do
    ...
    context "ending punctutation" do
      it "handles ending question marks" do
        ...
        Cadmium::Pragmatic.new.tokenize(text).should eq(["what", "is", "your", "name", "?"])
      end

      it "handles exclamation points" do
        text = "You are the best!"
        Cadmium::Pragmatic.new.tokenize(text).should eq(["you", "are", "the", "best", "!"])
      end

      it "handles periods" do
        text = "This way a productive day."
        Cadmium::Pragmatic.new.tokenize(text).should eq(["this", "way", "a", "productive", "day", "."])
      end

      it "handles quotation marks" do
        text = "\"He is not the one you are looking for.\""
        Cadmium::Pragmatic.new.tokenize(text).should eq(["\"", "he", "is", "not", "the", "one", "you", "are", "looking", "for", ".", "\""])
      end
      ...
      it "handles single quotation marks ('twas)" do
        ...
        Cadmium::Pragmatic.new.tokenize(text).should eq(["'twas", "the", "night", "before", "christmas", "and", "'twas", "cloudy", "."])
      end

      it "handles double quotes at the end of a sentence" do
        text = "She said, \"I love cake.\""
        Cadmium::Pragmatic.new.tokenize(text).should eq(["she", "said", ",", "\"", "i", "love", "cake", ".", "\""])
      end

      it "handles double quotes at the beginning of a sentence" do
        text = "\"I love cake.\", she said to her friend."
        Cadmium::Pragmatic.new.tokenize(text).should eq(["\"", "i", "love", "cake", ".", "\"", ",", "she", "said", "to", "her", "friend", "."])
      end

      it "handles double quotes in the middle of a sentence" do
        text = "She said, \"I love cake.\" to her friend."
        Cadmium::Pragmatic.new.tokenize(text).should eq(["she", "said", ",", "\"", "i", "love", "cake", ".", "\"", "to", "her", "friend", "."])
      end
    end
    ...
    context "other punctutation" do
      it "handles ellipses" do
        ...
        Cadmium::Pragmatic.new.tokenize(text).should eq(["today", "is", "the", "last", "day", "..."])
      end

      it "handles special quotes" do
        text = "«That's right», he said."
        Cadmium::Pragmatic.new.tokenize(text).should eq(["«", "that's", "right", "»", ",", "he", "said", "."])
      end

      it "handles upside down punctuation (¿)" do
        text = "¿Really?"
        Cadmium::Pragmatic.new.tokenize(text).should eq(["¿", "really", "?"])
      end
      ...
      it "handles colons" do
        ...
        Cadmium::Pragmatic.new.tokenize(text).should eq(["this", "was", "the", "news", ":", "'", "today", "is", "the", "day", "!", "'"])
      end

      it "handles web addresses" do
        text = "Please visit the site - https://www.tm-town.com"
        Cadmium::Pragmatic.new.tokenize(text).should eq(["please", "visit", "the", "site", "-", "https://www.tm-town.com"])
      end
      ...
      it "handles multiple dashes" do
        ...
        Cadmium::Pragmatic.new.tokenize(text).should eq(["john", "-", "here", "is", "your", "ticket", "."])
      end

      it "handles brackets" do
        text = "This is an array: ['Hello']."
        Cadmium::Pragmatic.new.tokenize(text).should eq(["this", "is", "an", "array", ":", "[", "'", "hello", "'", "]", "."])
      end

      it "handles double question marks" do
        text = "This is a question??"
        Cadmium::Pragmatic.new.tokenize(text).should eq(["this", "is", "a", "question", "?", "?"])
      end
      ...
      it "handles contractions 1" do
        ...
        Cadmium::Pragmatic.new.tokenize(text).should eq(["how'd", "it", "go", "yesterday", "?"])
      end

      it "handles contractions 2" do
        text = "You shouldn't worry."
        Cadmium::Pragmatic.new.tokenize(text).should eq(["you", "shouldn't", "worry", "."])
      end

      it "handles contractions 3" do
        text = "We've gone too far. It'll be over when we're done."
        Cadmium::Pragmatic.new.tokenize(text).should eq(["we've", "gone", "too", "far", ".", "it'll", "be", "over", "when", "we're", "done", "."])
      end

      it "handles numbers" do
        text = "He paid $10,000,000 for the new house which is equivalent to ¥1,000,000,000.00."
        Cadmium::Pragmatic.new.tokenize(text).should eq(["he", "paid", "$10,000,000", "for", "the", "new", "house", "which", "is", "equivalent", "to", "¥1,000,000,000.00", "."])
      end
      ...
      it "follows the Chicago Manual of Style on punctuation" do
        ...
        Cadmium::Pragmatic.new.tokenize(text).should eq(["an", "abbreviation", "that", "ends", "with", "a", "period", "must", "not", "be", "left", "hanging", "without", "it", "(", "in", "parentheses", ",", "e.g.", ")", ",", "and", "a", "sentence", "containing", "a", "parenthesis", "must", "itself", "have", "terminal", "punctuation", "(", "are", "we", "almost", "done", "?", ")", "."])
      end

      it "is case insensitive" do
        text = "his name is mr. smith, king of the 'entire' forest."
        Cadmium::Pragmatic.new.tokenize(text).should eq(["his", "name", "is", "mr.", "smith", ",", "king", "of", "the", "'", "entire", "'", "forest", "."])
      end

      it "handles web url addresses #1" do
        text = "Check out http://www.google.com/?this_is_a_url/hello-world.html for more info."
        Cadmium::Pragmatic.new.tokenize(text).should eq(["check", "out", "http://www.google.com/?this_is_a_url/hello-world.html", "for", "more", "info", "."])
      end

      it "handles web url addresses #2" do
        text = "Check out https://www.google.com/?this_is_a_url/hello-world.html for more info."
        Cadmium::Pragmatic.new.tokenize(text).should eq(["check", "out", "https://www.google.com/?this_is_a_url/hello-world.html", "for", "more", "info", "."])
      end

      it "handles web url addresses #3" do
        text = "Check out www.google.com/?this_is_a_url/hello-world.html for more info."
        Cadmium::Pragmatic.new.tokenize(text).should eq(["check", "out", "www.google.com/?this_is_a_url/hello-world.html", "for", "more", "info", "."])
      end

      it "handles email addresses" do
        text = "Please email example@example.com for more info."
        Cadmium::Pragmatic.new.tokenize(text).should eq(["please", "email", "example@example.com", "for", "more", "info", "."])
      end
    ...
    context "abbreviations" do
      it "handles military abbreviations" do
        ...
        Cadmium::Pragmatic.new.tokenize(text).should eq(["his", "name", "is", "col.", "smith", "."])
      end

      it "handles institution abbreviations" do
        text = "She went to East Univ. to get her degree."
        Cadmium::Pragmatic.new.tokenize(text).should eq(["she", "went", "to", "east", "univ.", "to", "get", "her", "degree", "."])
      end

      it "handles company abbreviations" do
        text = "He works at ABC Inc. on weekends."
        Cadmium::Pragmatic.new.tokenize(text).should eq(["he", "works", "at", "abc", "inc.", "on", "weekends", "."])
      end

      it "handles old state abbreviations" do
        text = "He went to school in Mass. back in the day."
        Cadmium::Pragmatic.new.tokenize(text).should eq(["he", "went", "to", "school", "in", "mass.", "back", "in", "the", "day", "."])
      end

      it "handles month abbreviations" do
        text = "It is cold in Jan. they say."
        Cadmium::Pragmatic.new.tokenize(text).should eq(["it", "is", "cold", "in", "jan.", "they", "say", "."])
      end

      it "handles miscellaneous abbreviations" do
        text = "1, 2, 3, etc. is the beat."
        Cadmium::Pragmatic.new.tokenize(text).should eq(["1", ",", "2", ",", "3", ",", "etc.", "is", "the", "beat", "."])
      end

      it "handles one letter abbreviations (i.e. Alfred E. Stone)" do
        text = "Alfred E. Stone is a person."
        Cadmium::Pragmatic.new.tokenize(text).should eq(["alfred", "e.", "stone", "is", "a", "person", "."])
      end

      it "handles repeating letter-dot words (i.e. U.S.A. or J.C. Penney)" do
        text = "The U.S.A. is a country."
        Cadmium::Pragmatic.new.tokenize(text).should eq(["the", "u.s.a.", "is", "a", "country", "."])
      end

      it "handles abbreviations that occur at the end of a sentence" do
        text = "He works at ABC Inc."
        Cadmium::Pragmatic.new.tokenize(text).should eq(["he", "works", "at", "abc", "inc."])
      end

      it "handles punctuation after an abbreviation" do
        text = "Exclamation point requires both marks (Q.E.D.!)."
        Cadmium::Pragmatic.new.tokenize(text).should eq(["exclamation", "point", "requires", "both", "marks", "(", "q.e.d.", "!", ")", "."])
      end
    end
  end
end
